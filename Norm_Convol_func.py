...
def normalize_convolve(image, kernel,file_name):
#Normalize
	...
	#cv2.waitKey(0)
	#path='H:/save data/'+str(file_name)+'.tif'
	#status = cv2.imwrite(path,normalizedImg)
...
# size (i.e., width and height) are not reduced
	pad = (kW - 1) // 2
	normalizedImg = cv2.copyMakeBorder(normalizedImg, pad, pad, pad, pad,
...
# *center* region of the current (x, y)-coordinates dimensions
			roi = normalizedImg[y - pad:y + pad + 1, x - pad:x + pad + 1]
